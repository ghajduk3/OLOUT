...
def evaluation_suite(phylogenetic_tree:str, distance_matrix, radial_visualization=constants.RADIAL_LAYOUT_BRANCH_LENGTH):
    ...
    figure_arguments.update({'title' : f'Unordered tree, pivot stress : {pivot_stress_unordered_tree:.5f}, adj nodes stress {adjacent_nodes_stress_unordered_tree:.3f}'})
    ...
    figure_arguments.update({'title': f"Phylogenetic tree pivot stress : {pivot_stress_ordered_no_correction:.3f}, adj nodes stress {adjacent_nodes_stress_ordered_no_correction:.3f}"})
    ...
    figure_arguments.update({'title': f"Phylogenetic tree leaf ordering , pivot stress : {pivot_stress_angle_correction:.3f}"})
    ...
    figure_arguments.update(
        {'title': f"Phylogenetic tree leaf ordering , ajdacent nodes stress : {adjacent_node_stress_angle_correction:.3f}"})
...
def run_evaluation_suites(recreate_data=False):
    ...
    for index, directory in enumerate(final_data_paths):
        ...
        try:
            evaluated_data = evaluation_suite(phylogenetic_newick_string, distance_matrix, radial_visualization=constants.RADIAL_LAYOUT_LEAF_COUNT)
...
def run_single_evaluation_suite(suite_name:str):
    ...
    data_json = json.load(json_file)
    nexus_file_url, phylogenetic_newick_string, distance_matrix, tree_node_mapping = data_json.values()
    evaluation_suite(phylogenetic_newick_string, distance_matrix, radial_visualization=constants.RADIAL_LAYOUT_LEAF_COUNT)
