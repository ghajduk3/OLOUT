...
class POLO:
    ...
    #     Add newick tree with @property tag
    def get_optimal_leaf_ordering(self):
        tree, mapping = Parser.parse_newick_tree(self.newick_tree)
        self.__get_optimal_ordered_tree(tree, self.distance_matrix)
        ordered_tree = self.__tree_reorder(tree)
        print(ordered_tree.pre_order())
    ...
    def __get_optimal_ordered_tree(self,v, distance_matrix):
        ...
        if children_number == 3:
            ...
            possible_permutations = get_permutations(list(range(children_number)),2)
            # Calculate the score for best permutation, left and right nodes
            best_permutation = (1000, 0, 0)
            ...
            print(f"Best permutation is with score {score}, with left nodes {[child.get_id() for child in left]} and with right nodes {[child.get_id() for child in right]}")
            dummy_node_left.children = left
            v.children = right_nodes
            v.children.insert(0,dummy_node_left)
            print(f"After choosing the best permutation v with id {v.get_id()} has children {[child.get_id() for child in v.get_children()]}")
    ...
    def __optimal_ordering(self, v, distance_matrix):
        ...
        if v.is_leaf():
            ...
            print(f"Node id {v.get_id()}, M {self.M}")
            for l in L:
                for r in R:
                    print(f"Outer loop, left node {l}, right node {r}, left {L}, right {R}, root id {v.get_id()}")
                    # self.M[left_node.get_id(),l,r] = self.__get_optimal_ordered_tree(left_node,distance_matrix)
                    # self.M[right_node.get_id(),l,r] = self.__get_optimal_ordered_tree(right_node, distance_matrix)
                    self.__get_optimal_ordered_tree(left_node,distance_matrix)
                    self.__get_optimal_ordered_tree(right_node, distance_matrix)
                for i in L:
                    for j in R:
                        print(f"Inner loop i: {i}, j: {j}, left {left_node.get_id()}, right {right_node.get_id()}, root {v.get_id()}, left nodes {L}, right nodes {R}")
                        for g in R:
                            self.temp[i,g] = min(self.M.get((left_node.get_id(),i,h),10000000) + self.distance_matrix[h,g] for h in L)
                        self.M[v.get_id(),i,j] = self.M[v.get_id(),j,i] = min(self.temp[i,g] + self.M.get((right_node.get_id(),g,j),10000000) for g in R)
                    # return self.M[v.get_id(),i,j]
    ...
    def __tree_reorder(self, v):
        ...
        if len(L) and len(R):
            def getkey(z):  ##added, this is to replace a lambda function
                u, w = z
                return self.M.get((v.id, u, w),1000000)
            if len(L) and len(R):
