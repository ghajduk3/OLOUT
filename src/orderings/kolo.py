            ...
            for l in L:
                ...
                for i in L:
                    for j in R:
                        ...
                        for g in R:
                            self.temp[i,g] = min(self.M.get((left_node.get_id(),i,h),10e10) + self.distance_matrix[h,g] for h in L)
                        self.M[v.get_id(),i,j] = self.M[v.get_id(),j,i] = min(self.temp[i,g] + self.M.get((right_node.get_id(),g,j),10e10) for g in R)
