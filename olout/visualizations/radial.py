...
class RadialLayoutLeafCount:
    def __init__(self, tree):
        """
        Class that represents RadialLayoutAlgorithm that uses Leaf Count as the heuristic - RLALC
    ...
    def get_radial_layout_coordinates_FFAC(self):
        corrected_coordinates, best_correction_factor = self._apply_angle_corrections_FFAC()
    ...
    def get_radial_layout_coordinates_ANBC(self):
        corrected_coordinates, counter = self._apply_angle_corrections_ANBC()
        stress_all_node_pairs = self._calculate_stress_all_node_pairs(corrected_coordinates)
    ...
    def _apply_angle_corrections_FFAC(self):
        ...
        for correction_factor in correction_factors:
            corrected_coordinates = self._calculate_angle_correction_coordinates(copy.deepcopy(self.coordinates),
                                                                                 correction_factor, levels,
                                                                                 self.tree.get_id())
        ...
        return self._calculate_angle_correction_coordinates(copy.deepcopy(self.coordinates), best_correction_factor,
                                                            levels, self.tree.get_id()), best_correction_factor
    ...
    def _apply_angle_corrections_ANBC(self):
        ...
        for index, node in enumerate(leaf_ordering):
            ...
            correction_factors = np.arange(1, 10, 0.2).tolist() + np.arange(30, 35, 0.2).tolist() + np.arange(50, 55,
                                                                                                              0.2).tolist() + np.arange(
                100, 105, 0.2).tolist()
            stresses = []
            for correction_factor in correction_factors:
                angle = self.tau[node] + self.omega[node] / correction_factor
    ...
    def _calculate_stress_all_node_pairs(self, coordinates):
        ...
        for node, adj_node in itertools.combinations(internal_leaf_ordering, 2):
            ...
            air_distance = max(RadialLayoutLeafCount._get_euclidian_distance(coordinates[node], coordinates[adj_node]),
                               0.00000001)
