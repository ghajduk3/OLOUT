    ...
    def __optimal_ordering(self, v, distance_matrix):
        ...
        if v.is_leaf():
            ...
            for i in L:
                for j in R:
                    self.temp[i, j] = min(
                        self.M.get((left_node.get_id(), i, h), 10e10) + self.distance_matrix[h, j] for h in L)
                    self.M[v.get_id(), i, j] = self.M[v.get_id(), j, i] = min(
                        self.temp[i, j] + self.M.get((right_node.get_id(), l, j), 10e10) for l in R)
    ...
    def __tree_reorder(self, v):
        ...
        if len(L) and len(R):
            ...
            if w in leaves(v.get_right().get_left()):
                v.children[1].children[1], v.children[1].children[
                    0] = v.get_right().get_left(), v.get_right().get_right()
